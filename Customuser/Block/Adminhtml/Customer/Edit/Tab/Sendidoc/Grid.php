...
class Magentothem_Customuser_Block_Adminhtml_Customer_Edit_Tab_Sendidoc_Grid extends Mage_Adminhtml_Block_Widget_Grid
{
    /**
     * Set grid params
     *
    ...
    {
        parent::__construct();
        $this->setId('customer_sendidoc_grid');
        ...
        if ($customerId) {
            ...
            if($rolesDetail['role_name'] == 'Key User' && $rolesDetail['role_type'] == 'G') { 
                ...
                if ($salesAreaIds) {
                    ...
                    ->where('main_table.cust_id = '.$customerId.' AND sa.website_id = "'.$websiteId.'"');
    ...
    protected function _prepareColumns()
    {
        ...
        if($websiteCode == Magentothem_Usermanagement_Helper_Data::NA_WEBSITE) {
            $this->addColumn(
                'is_default',
                array(
        ...
        $this->addColumn(
            'sap_cust_id',
            array(
                'header'    => Mage::helper('customer')->__('Sold To'),
                'index'     => 'sap_cust_id'
            )
        );
