    ...
    public function initForm()
    {
        ...
        if (!$this->getRequest()->getParam('partner')) {
            $fieldset->addField(
                ...
                'text',
                array(
                     'label'     => Mage::helper('axaltacore_customuser')->__('Username'),
                     'disabled' => TRUE,
                     'name'      => 'uid',
                )
            );
        ...
        if ($customer->getId()) {
            ...
            // do not change for sonar. it is magento default
        ...
        foreach ($attributes as $attribute) {
            ...
            /* As raul wants privileges to update firstname / lastname */
            if (!($this->getRequest()->getParam('partner')) && $attrId == 'email') {
                $form->getElement($attrId)->setDisabled('disabled');
            }
